...
PokemonP2 = ['Venusaur',['Solar Beam','Poison Powder','Sleep Powder','Take Down']]
...
def Damaging(Move,PokemonP1,PokemonP2): #PokemonP1 = ['Charizard',[Moves],[usage_move]]
    ...
    critical = movec(Move)
    print(Pokemon[PokemonP1[0]]['Moves'][Move]['Type'])
    print(Pokemon[PokemonP1[0]]['Type'])
    if Move in PokemonP1:
        ...
        damage = (((((((((2*50)/5)+2)*Type(Pokemon[PokemonP1[0]],Move,Pokemon[PokemonP2[0]]))*Atk(PokemonP1[0]))/Def(PokemonP2[0]))+((100*Type(Pokemon[PokemonP1[0]],Move,Pokemon[PokemonP2[0]]))/Pokemon[PokemonP1[0]]['Moves'][Move]['pwr']))/50))*Modifier
        ##damage = ((((((((2*50)/5)+2)*power)*Atk(p1))/Def(p2))/50)+2)*Modifier
        print(dmg)
    else :
        damage = ((((((((2*50)/5)+2)*power)*Atk(PokemonP2[0]))/Def(PokemonP1[0]))/50)+2)
#=====================================================================================================================================
""" 
...
def Type(p1,usage_move,p2): ##Type Calculator
    ...
    if p1movetype in Pokemon[p2]['Weakness']:#usage move == ชื่อท่า
        dmg = int(Pokemon[p2]['Weakness'][p1movetype])*int(Pokemon[p1]['Moves'][usage_move]['pwr'])
    elif p1movetype in Pokemon[p2]['Resistant']:
        dmg = int(Pokemon[p2]['Resistant'][p1movetype])*int(Pokemon[p1]['Moves'][usage_move]['pwr'])
    elif p1movetype in Pokemon[p2]['Immune']:
        dmg = int(Pokemon[p2]['Immune'][p1movetype])*int(Pokemon[p1]['Moves'][usage_move]['pwr'])
    else:
